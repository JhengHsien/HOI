...
o_appearance = {}
triplets_repeat = {}
for i in hico_text_label_list:
    triplets_repeat[i] = True

for item in hico_text_label_list:
    if item[0] not in v_appearance:
        v_appearance[item[0]] = 1
    else:
        v_appearance[item[0]] += 1
    if item[1] not in o_appearance:
        o_appearance[item[1]] = 1
    else:
        o_appearance[item[1]] += 1

for item in hico_text_label_list:
    if v_appearance[item[0]] >= 2:
        triplets_repeat[item] = False
    if o_appearance[item[1]] >= 2:
        triplets_repeat[item] = False

print(v_appearance)
print(o_appearance)

# for item in triplets_repeat:
#     if (triplets_repeat[item] == True): print(item)
#     if (triplets_repeat[item] == False): print(item)
